    ...
    getSelectAddress(){
        if(sessionStorage['addressId']!==undefined){
            ...
            request(sUrl).then(res=>{
                if(res.code===200){
                    this.setState({sName:res.data.name,sCellphone:res.data.cellphone,
                        sProvince:res.data.province,sCity:res.data.city,sArea:res.data.area,sAddress:res.data.address,})
                }
            })
        }
    ...
    submitOrder(){
        ...
        if(sAddressId!==undefined){
            if(this.props.state.cart.total>0){
                ...
                Toast.info("您的购物车是空的~",1.7)
            }
        }else{
            Toast.info("请先填写收货地址哦",1.7)
        }
    }
    render(){
        return(
            <div className={Css['page']}>
                            ...
                            sessionStorage['addressId']!==undefined || localStorage['addressId']!==undefined?
                                <React.Fragment>
                                    ...
                                    <div className={Css["address"]}>
                                        ...
                                        <span>{this.state.sProvince} {this.state.sCity} {this.state.sArea}</span>
                                    </div>
                                </React.Fragment>
                            :
                                        ...
                                        item.checked?
                                            <div key={index} className={Css["goods-list"]}>
                                                ...
                                                <div className={Css["goods-param"]}>
                                                    ...
                                                    <div className={Css["price"]}>¥ {item.price*item.amount}</div>
                                                </div>
                                            </div>
                                        :''
                ...
                <div className={Css["fixed-total-wrap"]}>
                ...
                <div className={Css["balance-wrap"]}>
                    <div className={Css["price-wrap"]}>
                        <div className={Css["actual-price"]}>实际金额：<span>¥ {this.props.state.cart.total+this.props.state.cart.freight/*parseFloat(Math.round(this.props.state.cart.total+this.props.state.cart.frieght).toFixed(3))*/}</span></div>
                    </div>
