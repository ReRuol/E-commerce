...
import Css from '../../../assets/css/home/search/search.module.css';
...
export default class  SearchItems extends React.Component{
    ...
    //选择价格范围
    checkedPrice(index,fPrice1,fPrice2){
        let aPrice=this.state.aPrice;
            ...
            <div className={Css['page']}>
                <div className={Css['search-top']}>
                    <div className={Css['search-head']}>
                        ...
                        <div className={Css['search-wrap']} onClick={this.getStyle.bind(this,{"display":"block"})}>
                            <div className={Css['search-icon']}></div>
                            <div className={Css['search-text']}>输入宝贝名称</div>
                        </div>
                    ...
                    <div className={Css['order-main']}>
                        <div className={this.state.bPriceMenu?Css['order-item']+' '+Css['active']:Css['order-item']} onClick={this.handlePriceOrder.bind(this)}>
                            ...
                            <ul className={this.state.bPriceMenu?Css['order-menu']:Css['order-menu']+" "+Css['hide']}>
                                {
                ...
                <div ref="mask" className={this.state.bMask?Css["mask"]:Css["mask"]+" "+Css['hide']} onClick={this.hideScreen.bind(this)}></div>
                <div ref="screen" className={Css["screen"]+" "+this.state.screenMove}>
                    ...
                    <div className={Css['attr-wrap']}>
                        <div className={Css['attr-title-wrap']} onClick={this.handleClassify.bind(this)}>
                            ...
                            <div className={this.state.aClassify.checked?Css['attr-icon-down']:Css['attr-icon-up']}></div>
                        </div>
                        <div className={this.state.aClassify.checked?Css["item-wrap"]:"hide"}>
                            {
                        ...
                        <div className={Css['attr-title-wrap']} onClick={this.handlePrice.bind(this)}>
                            ...
                            <div className={Css['price-wrap']}>
                                ...
                                <div className={Css["price-input"]} onClick={this.preventBubble.bind(this)}>
                                    <input type="tel" placeholder="最高价" value={this.state.fPrice2===0?"":this.state.fPrice2}
                                    onChange={this.changePrice2.bind(this)}></input>
                                </div>
                            </div>
                            <div className={this.state.aPrice.checked?Css['attr-icon-down']:Css['attr-icon-up']} ></div>
                        </div>
                        <div className={this.state.aPrice.checked?Css["item-wrap"]:"hide"}>
                            {
                                this.state.aPrice.items.map((item,index)=>{
                                    return (
                                    <div onClick={this.checkedPrice.bind(this,index,item.price1,item.price2)} key={index}
                                    className={item.checked?Css['item']+" "+Css["active"]:Css['item']}>{item.price1}-{item.price2}</div>
                                    )
                                })
                            }
                        </div>
                        <div style={{width:'100%',height:'0.9rem',backgroundColor:'#efefef'}}></div>
                        {
                                    ...
                                    return (
                                      <React.Fragment key={index}>
                                        <div className={Css['attr-title-wrap']} onClick={this.handleAttr.bind(this,index)}>
                                            ...
                                            <div className={item.checked?Css['attr-icon-down']:Css['attr-icon-up']}></div>
                                        </div>
                                        <div className={item.checked?Css["item-wrap"]:Css["item-wrap"]+'height-hide'}>
                                            {
                                                item.param.length>0?
                                                    item.param.map((item2,index2)=>{
                                                        ...
                                                        onClick={this.checkedParams.bind(this,index,index2)}>
                                                            {item2.title}
                                                        </div>
                                                        )
                                                    })
                    ...
                    <div style={{width:'100%',height:'2%',backgroundColor:'#efefef'}}></div>
                    <div style={{width:'100%',height:'2.5rem',backgroundColor:'#ffffff'}}></div>
                    </div>
                ...
                <SearchComponent pageStyle={this.state.pageStyle} childStyle={this.getStyle.bind(this)}
                 isLocal="1" childKeywords={this.getChildKeywords.bind(this)} keywords={this.state.sKeywords}></SearchComponent>
            </div>
        );
    }
}